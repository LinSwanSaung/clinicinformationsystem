...
const VitalCard = memo(({ title, value, unit, delta, chartData, dataKey, color: _color }) => {
  ...
  return (
    <motion.div
      ...
      className="group"
    >
      <Card className="border-border/60 h-full transition-colors hover:border-border">
        <CardContent className="space-y-3 p-4">
          ...
          <div className="h-[180px]" role="img" aria-label={`${title} trend chart`}>
            {!hasValidData ? (
              <div className="border-muted-foreground/30 flex h-full items-center justify-center rounded-md border border-dashed text-sm text-muted-foreground">
                Not enough data
              </div>
            ) : (
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-baseline justify-between">
              ...
              {combinedDelta && combinedDelta.value !== null && (
                <div
                  className={`flex items-center gap-1 text-xs font-medium ${combinedDelta.color}`}
                >
...
const VitalsSnapshot = ({ visits, loading, error, onRetry }) => {
  ...
  const hrDelta = useMemo(
    () => calculateDelta(latest?.heartRate, previous?.heartRate, false),
    [latest, previous]
  );
  const tempDelta = useMemo(
    () => calculateDelta(latest?.temperature, previous?.temperature, true),
    [latest, previous]
  );
  const weightDelta = useMemo(
    () => calculateDelta(latest?.weight, previous?.weight, false),
    [latest, previous]
  );
  ...
  if (!visits?.length || !latest) {
    return (
      <Card className="border-border/80 border shadow-sm">
        ...
        <CardContent>
          <div className="border-muted-foreground/30 bg-muted/10 space-y-3 rounded-lg border border-dashed p-6 text-center">
            <Activity className="mx-auto h-8 w-8 text-muted-foreground" aria-hidden="true" />
            <p className="text-base font-medium text-foreground">
              {t('patient.vitalsSnapshot.emptyTitle')}
            </p>
            <p className="text-sm text-muted-foreground">
              {t('patient.vitalsSnapshot.emptyDescription')}
            </p>
          </div>
        </CardContent>
      </Card>
