 ...
 */
router.get('/visit/:visitId', authenticate, authorize('doctor', 'nurse'), async (req, res) => {
  try {
...
router.post('/', authenticate, authorize('doctor'), requireActiveVisit, async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ success: true, data: diagnosis, message: 'Diagnosis created successfully' });
  } catch (error) {
    res.status(400).json({
      success: false,
      message: error.message,
