...
const PatientMedicalRecordManagement = () => {
  ...
  const handleAddAllergy = () => {
    if (!hasActiveVisit) {
      showWarning(
        'Cannot add allergy: Patient does not have an active visit. Please start a consultation first.'
      );
      return;
    }
  ...
  const handleSaveAllergy = async () => {
    ...
    try {
      ...
      if (error.response?.data?.code === 'NO_ACTIVE_VISIT') {
        showWarning(
          'Security Check Failed: Cannot add allergy because patient has no active visit.'
        );
      } else {
        showError(
  ...
  const handleAddDiagnosis = () => {
    if (!hasActiveVisit) {
      showWarning(
        'Cannot add diagnosis: Patient does not have an active visit. Please start a consultation first.'
      );
      return;
    }
  ...
  return (
    <PageLayout
      ...
      subtitle="Complete patient medical record management (Doctor Access)"
      fullWidth={true}
    >
      ...
      >
        <AllergyForm allergy={newAllergy} onChange={setNewAllergy} disabled={loading} />
      </FormModal>
      ...
      >
        <DiagnosisForm diagnosis={newDiagnosis} onChange={setNewDiagnosis} disabled={loading} />
      </FormModal>
    </PageLayout>
  );
};
