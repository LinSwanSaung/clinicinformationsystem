...
const NurseDashboard = () => {
  ...
  useEffect(() => {
    const loadPatients = async () => {
      try {
        const response = await patientService.getNursePatients();
        if (response.success) {
  ...
  const handleMarkReady = async (patientId) => {
    try {
      await patientService.markPatientReady(patientId);
  ...
  const handleDelayPatient = async (patientId, reason) => {
    try {
      await patientService.addPatientDelay(patientId, reason);
      setPatients(patients.map(p => 
  ...
  const handleSaveVitals = async (patientId, vitals, notes) => {
    try {
      await patientService.updatePatientVitals(patientId, { vitals, notes });
      setPatients(patients.map(p => 
        p.id === patientId ? { 
          ...p, 
  ...
  const handleRemoveDelay = async (patientId) => {
    try {
      await patientService.removePatientDelay(patientId);
      setPatients(patients.map(p => 
