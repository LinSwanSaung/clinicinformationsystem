...
import { getUsersByIds, getPaymentWithRelations } from './repositories/BillingRepo.js';
  ...
  async generateReceiptPDF(paymentId) {
    try {
      ...
      if (payment.received_by) {
        const cashiers = await getUsersByIds([payment.received_by]);
        payment.received_by_user = cashiers?.[0] || null;
      }
      ...
      doc.fontSize(10).text(
        `Date: ${new Date(payment.received_at).toLocaleString('en-US', {
          year: 'numeric',
          month: 'long',
          day: 'numeric',
          hour: '2-digit',
      ...
      doc.fontSize(10);
      doc.text(
        `Name: ${payment.invoice?.patient?.first_name || ''} ${payment.invoice?.patient?.last_name || ''}`
      );
      ...
      doc.text(`Amount Paid: $${payment.amount?.toFixed(2) || '0.00'}`, {
        fontSize: 12,
        bold: true,
      });
      ...
      doc.fontSize(10);
      doc.text(
        `Cashier: ${payment.received_by_user?.first_name || ''} ${payment.received_by_user?.last_name || 'N/A'}`
      );
