...
export async function fetchDispenses(filters = {}) {
	const { from, to, search, page, pageSize, sortBy, sortDir } = DispenseFiltersSchema.parse(filters);
	...
	const composed = invoiceItems
		...
		.map((it) => {
			...
			return {
				...
				dispensedBy: user
					? { userId: user.id, name: `${user.first_name || ''} ${user.last_name || ''}`.trim(), role: user.role }
					: null,
