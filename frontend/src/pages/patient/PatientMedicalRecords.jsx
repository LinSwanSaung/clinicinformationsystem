...
const PatientMedicalRecords = () => {
  ...
  const loadPatientData = async () => {
    try {
      ...
      const [allergiesRes, diagnosesRes, visitsRes, vitalsRes, prescriptionsRes] =
        await Promise.all([
          ...
          patientPortalService.getPrescriptions(true).catch((err) => {
            // Include all prescriptions (active + inactive)
  ...
  if (error) {
    return (
      <PageLayout title="My Medical Records" subtitle="View your complete medical history">
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-6">
          {activeTab === 'overview' && (
            <>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg font-semibold">
                      {t('patient.medicalRecords.latestVitals')}
                    </h3>
                    {latestVitals.recorded_at && (
                      <span className="text-sm text-muted-foreground">
                        {t('patient.medicalRecords.recordedOn')}{' '}
              ...
              {visits.length > 0 && (
                <div className="space-y-4">
                  <h3 className="text-lg font-semibold">
                    {t('patient.medicalRecords.recentVisits')}
                  </h3>
          ...
          {activeTab === 'prescriptions' && (
            <div className="space-y-4">
              <h3 className="text-lg font-semibold">
                {t('patient.medicalRecords.prescriptionHistory')}
              </h3>
              {prescriptions.length > 0 ? (
                <div className="grid gap-4">
                  {prescriptions
                    ...
                    .map((prescription) => {
                      const isActive =
                        prescription.status === 'active' || prescription.is_active === true;
                      return (
                        <div
                          key={prescription.id}
                          className={`space-y-3 rounded-lg border p-4 transition-all ${
                            isActive
                              ? 'border-green-500 bg-green-50 shadow-md ring-2 ring-green-200 dark:border-green-700 dark:bg-green-950/30 dark:ring-green-800'
                              ...
                              <div className="grid gap-2 text-sm">
                                <div className="flex items-center gap-2">
                                  <span className="font-medium text-muted-foreground">
                                    {t('patient.prescriptions.dosage')}:
                                  </span>
                                ...
                                {prescription.frequency && (
                                  <div className="flex items-center gap-2">
                                    <span className="font-medium text-muted-foreground">
                                      {t('patient.prescriptions.frequency')}:
                                    </span>
                                ...
                                {prescription.duration && (
                                  <div className="flex items-center gap-2">
                                    <span className="font-medium text-muted-foreground">
                                      {t('patient.prescriptions.duration')}:
                                    </span>
              ...
              {allergies.length > 0 ? (
                <div className="grid gap-4">
                  {allergies.map((allergy) => (
                    <div key={allergy.id} className="space-y-2 rounded-lg border p-4">
                      <div className="flex items-start justify-between">
                        <div>
                      ...
                      {allergy.reaction && (
                        <p className="text-sm">
                          <strong>{t('patient.medicalRecords.reaction')}:</strong>{' '}
                          {allergy.reaction}
                        </p>
                      )}
                      {allergy.notes && (
          ...
          {activeTab === 'diagnoses' && (
            <div className="space-y-4">
              <h3 className="text-lg font-semibold">
                {t('patient.medicalRecords.medicalDiagnoses')}
              </h3>
              {diagnoses.length > 0 ? (
                <div className="grid gap-4">
                  {diagnoses.map((diagnosis) => (
                    <div key={diagnosis.id} className="space-y-2 rounded-lg border p-4">
                      <div className="flex items-start justify-between">
                        <div>
                      ...
                      {diagnosis.diagnosed_date && (
                        <p className="text-sm text-muted-foreground">
                          {t('patient.medicalRecords.diagnosed')}:{' '}
                          {new Date(diagnosis.diagnosed_date).toLocaleDateString()}
                        </p>
                      )}
                      {diagnosis.notes && <p className="text-sm">{diagnosis.notes}</p>}
                    </div>
                  ))}
                </div>
              ) : (
