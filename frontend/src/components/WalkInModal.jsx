...
const WalkInModal = ({ isOpen, onClose, onSubmit }) => {
  ...
  const searchPatients = async (searchTerm) => {
    ...
    try {
      ...
      const response = await apiService.get(`/patients/search/${encodeURIComponent(searchTerm.trim())}?limit=10`);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleModalClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto p-0">
          ...
          {step === 1 && (
            ...
            >
              <div className="space-y-3">
                <p className="text-sm font-medium mb-2">Search Patient</p>
              ...
              {patientSearch && patients.length === 0 && !isLoadingPatients && patientSearch.length >= 2 && (
            ...
            >
                ...
                <Alert variant="destructive">
                  ...
                  <AlertDescription>
                    <div className="space-y-2">
                      <div>
                        <strong>Warning:</strong> This patient already has an active appointment today.
                      </div>
                      <div className="text-sm">
                        Status: <Badge variant="secondary" className="text-xs">
                          {activeAppointmentDetails.status?.replace('-', ' ') || 'Active'}
                        </Badge>
                        {activeAppointmentDetails.doctor_name && (
                          <span className="ml-2">with {activeAppointmentDetails.doctor_name}</span>
                        )}
                      </div>
                      <Button 
                        variant="outline" 
              ...
              {bypassCheck && patientHasActiveAppointment && (
                <Alert variant="default">
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription>
                    <strong>Override Active:</strong> Proceeding despite existing appointment. The patient will have multiple appointments today.
                  </AlertDescription>
                </Alert>
              )}
              ...
              {capacityWarning && selectedDoctor && (
                <Alert variant={capacityWarning.type === 'error' ? 'destructive' : 'default'}>
                  ...
                  <AlertDescription>
                    <div className="space-y-2">
                      <div>
                        <strong>{capacityWarning.type === 'error' ? 'Cannot Accept Walk-In:' : 'Capacity Warning:'}</strong>
                      </div>
                      <div>{capacityWarning.message}</div>
                      {capacityWarning.details && (
                        <div className="text-sm mt-2 space-y-1">
                          <div>• Current Queue: {capacityWarning.details.currentQueue} patient(s)</div>
                          <div>• Available Slots: {capacityWarning.details.availableSlots}</div>
                          {capacityWarning.details.remainingTime && (
                            <div>• Remaining Time: {capacityWarning.details.remainingTime} minutes</div>
                          )}
                    ...
                    {doctors.map((doctor) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="space-y-2">
                            <div className="flex items-center justify-between">
                              <Badge className={`${doctor.statusColor || getStatusColor(doctor.status)} flex items-center gap-1`}>
                                <StatusIcon className="h-3 w-3" />
                                {doctor.statusText || doctor.status.charAt(0).toUpperCase() + doctor.status.slice(1).replace('-', ' ')}
                              </Badge>
                            ...
                            {doctor.currentlyWith && (
                              <p className="text-xs text-muted-foreground">
                                Currently with: {doctor.currentlyWith.patient?.first_name} {doctor.currentlyWith.patient?.last_name}
                              </p>
                            )}
        ...
        <div className="flex justify-between pt-4 border-t mx-6 pb-6">
          <Button
            variant="outline"
            onClick={step === 1 ? handleModalClose : handleBack}
          >
            {step === 1 ? 'Cancel' : 'Back'}
          </Button>
          ...
          {step === 2 && (
            <Button
              ...
              variant={(patientHasActiveAppointment && !bypassCheck) || (doctorCapacity && !doctorCapacity.canAccept) ? "secondary" : "default"}
            >
              {isSubmitting ? (
