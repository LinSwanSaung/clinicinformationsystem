...
const PatientAccountRegistration = () => {
  ...
  const handleBindByQuery = async () => {
    ...
    const matchFromSuggestions = bindSuggestions.find((patient) => {
      ...
      const nameMatch =
        [patient.first_name, patient.last_name].filter(Boolean).join(' ').trim().toLowerCase() ===
        normalizedQuery;
  ...
  const handleCreateAccount = async (event) => {
    ...
    try {
      ...
      setCreateSuccess(
        `Created patient account for ${createForm.first_name} ${createForm.last_name}`
      );
  ...
  const handleEditAccount = async (event) => {
    ...
    try {
      ...
      setEditSuccess(
        `Successfully updated account for ${editForm.first_name} ${editForm.last_name}`
      );
      closeEditDialog();
    } catch (err) {
  ...
  const handleBindAccount = async () => {
    ...
    setBinding(true);
    try {
      const response = await patientAccountService.bindAccount(
        selectedAccount.id,
        selectedPatient.id
      );
    ...
    >
      <div className="space-y-8">
        <Card>
          <CardHeader className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <CardDescription>
                {totalAccounts} accounts found. Use the search box to locate accounts by name or
                email.
              </CardDescription>
            </div>
            <div className="flex items-center gap-2">
              ...
              <Button
                variant="outline"
                onClick={() => loadAccounts(debouncedSearch)}
                disabled={loading}
              >
                Refresh
              </Button>
          ...
          <CardContent className="space-y-4">
            <div className="max-w-sm">
              <SearchBar
                ...
                ariaLabel="Search patient accounts"
              />
            </div>
            ...
            <DataTable
              columns={[
                ...
                },
                {
                  key: 'phone',
                  label: 'Phone',
                  render: (_, row) => row.phone || 'Not provided',
                },
                {
                  key: 'linked',
                ...
                },
                {
                  ...
                  render: (_, row) => (
                    <div className="flex gap-2">
                      ...
                      <Button size="sm" variant="secondary" onClick={() => openBindDialog(row)}>
                        <LinkIcon className="mr-2 h-4 w-4" />
                        {row.patient ? 'Re-link' : 'Bind'}
                      </Button>
                      {row.patient && (
        ...
        <FormModal
          ...
          description="Generate login credentials for a patient. They will still need to link their account with the correct patient record."
          onSubmit={handleCreateAccount}
          submitText={createSubmitting ? 'Creating...' : 'Create Account'}
          cancelText="Cancel"
        ...
        >
          ...
          <div className="space-y-1.5">
            ...
            {bindLookupError && <p className="text-sm text-red-600">{bindLookupError}</p>}
            ...
            <DialogHeader>
              ...
              <DialogDescription>
                {selectedAccount ? (
                  <>
                    Link{' '}
                    <strong>{`${selectedAccount.first_name} ${selectedAccount.last_name}`}</strong>{' '}
                    to a patient record.
                  </>
                ) : (
        ...
        <FormModal
          ...
          description={
            editingAccount
              ? `Update account information for ${editingAccount.first_name} ${editingAccount.last_name}.`
              : 'Update account information.'
          }
          onSubmit={handleEditAccount}
          submitText={editSubmitting ? 'Updating...' : 'Update Account'}
          cancelText="Cancel"
